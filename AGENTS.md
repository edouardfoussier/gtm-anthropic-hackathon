...
| Graph           | **Vanilla Three.js** mounted in a React component, behind a **data-only props interface** (`PersonNode[]` — statuses, seniority, reportsTo) — renderer stays swappable if 3D melts down. Layout **LOCKED (D016)**: people-only org constellation — each person a seniority-sized particle mini-sphere, reporting-line links, Claude's pick ignites orange progressively. Handover doc: `GRAPH-GUIDE.md`. |
...
- **D016** — 2026-07-09 — Graph layout **LOCKED** (closes the open item from D013): **people-only org constellation** — every person a particle mini-sphere sized by role seniority, hierarchical-radial placement from `reportsTo` links, Claude's pick ignites orange particle-by-particle with flow dots on its reporting line, others dim. No bloom/additive glow ever (physically invisible on `#F7F7F5` — verified in the effects research). Data contract: `PersonNode[]` snapshots diffed by the renderer (`components/graph/`). Handover doc: `GRAPH-GUIDE.md` (repo root). Supersedes D015's dot-particle-sphere graph description (the big company sphere was cut after visual review); the D015 palette/design system stands.
- **D017** — 2026-07-09 — Agent orchestration built as a decoupled pure-TypeScript module in `agentic/` (no `next/*` imports), not scattered in `lib/`. Core = `runProspect(input,{onEvent,signal})`: emits a typed `AgentEvent` stream (UI seam) + persists `ProspectState` to `data/prospects/{id}.json` (pipeline seam). `agentic/run.ts` = standalone entry + smoke. (Renumbered from an earlier D009 to avoid colliding with main's D009.)
- **D018** — 2026-07-09 — Agent flow is **Sillage-first**: Sillage people+signals → one Claude structured pick (`generateText({ output: Output.object })`) → FullEnrich enriches the pick. A `hot` signal → `signal_detected` (red node). Every step degrades to a deterministic fallback (no key → scored pick; adapter failure/timeout → mock) so a run never hangs or throws.
- **D019** — 2026-07-09 — SDK reality: the MCP client is in **`@ai-sdk/mcp`** (not `ai@7`); structured output = `generateText({ output: Output.object({ schema }) })`; Zod is **v4**. Sillage exposes BOTH a **V2 REST API** (`https://api.getsillage.com`, `Authorization: Bearer sk_live_…`) AND an **MCP server** (`/api/mcp/v2`, OAuth) — the headless adapter uses REST, interactive/agentic use uses MCP. Corrects the earlier "no public REST API" note in D003. Memory persisted 3 ways: `data/prospects/{id}.json`, `{id}.events.jsonl`, `data/seen.json`.
- **D020** — 2026-07-09 — Integration reality (agentic layer landed on main via an isolated worktree + PR): (a) the shipped UI is a **jury-CRM → generate-video** flow driven by `engine/data/jury.json` (keyed by `id`) + polled `Job` progress — **no `/api/prospect`, SSE, or live contact graph** — so the agentic layer currently serves as a **data engine** (enriched jury data + real signals); live-graph wiring is deferred. (b) The engine's `POST /generate` takes only `{prospectId, voiceId?}`, resolves the member from `engine/data/jury.json`, and **drafts the deck itself**. (c) Jury pre-bake: 13 companies mapped via Sillage MCP + all 18 members enriched via FullEnrich; PII cached locally, git-ignored under `data/jury-*`.
